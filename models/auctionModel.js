...
class AuctionModel {
    ...
    /**
     * Deletes an auction and its associated vehicle in a transaction
     * @param {number} auctionId 
     * @param {number} vehicleId 
     * @returns {Promise<Object>} Result object with success status
     */
    static async deleteAuctionWithVehicle(auctionId, vehicleId) {
        const client = await pool.connect();
        try {
            await client.query('BEGIN');

            // 1. Delete auction record
            await client.query(
                'DELETE FROM auction_vehicles WHERE auction_id = $1',
                [auctionId]
            );

            // 2. Use existing deleteVehicle API with transaction client
            const vehicleDeleted = await InventoryModel.deleteVehicle(vehicleId, client);

            if (!vehicleDeleted) {
                throw new Error('Failed to delete associated vehicle');
            }
            ...
            return {
                success: true,
                imagesDeleted: vehicleDeleted.imagesDeleted
            };

        } catch (error) {
            await client.query('ROLLBACK');
            console.error('Transaction error in deleteAuctionWithVehicle:', error);
            throw error;
        } finally {
            client.release();
        }
        ...
        try {
            ...
            query += `
                GROUP BY
                    ...
                    v.year, v.vin, v.price, v.mileage, v.exterior_color, v.interior_color,
                    v.transmission, v.fuel_type, v.body_type, v.engine, v.condition,
                    v.status, v.description, v.stock_number, v.location, v.carfax_link,
                    av.purchase_date, av.purchase_price, av.additional_costs,
                    av.total_investment, av.list_price, av.sold_price, av.status,
                    av.profit, av.created_at, av.updated_at, vi.image_urls, vi.primary_image_index
                ORDER BY ${dbSortBy} ${actualSortOrder}
